...
🟢 **Buses, paradas, arribos (TEA) y playas validados end-to-end contra las APIs reales** (2026-07-05). Specs en `spec/`: manual de conexión v1.0 + doc del servicio de transporte (`/api/transportepublico` v1.0.0).
...
- [x] `arribos()` (TEA, `/buses/busstops/{id}/upcomingbuses`) — **validado e2e**: devuelve `eta` en minutos + `distancia`, `posicion` y la ubicación del bus en camino. La doc oficial muestra un modelo incompleto (sin esos campos); el shape está fijado contra la respuesta real.
- [x] `paradas()` (**validado e2e**) y `variantes()` — contra la doc oficial.
- [x] `lineasPorParada()` — implementado según la doc, pero ⚠️ la API real respondió HTTP 400 en todas las paradas probadas (2026-07-05): endpoint posiblemente fuera de servicio.
